...
class MonthlyDataScraper(BaseEDBScraper):
    ...
    def insert_data(self, data: pd.DataFrame) -> None:
        ...
        # Use Azure data tracker's smart update
        smart_update(
...
def run_pipeline(use_managed_identity: bool = True, 
                storage_account: Optional[str] = None, 
    ...
    Args:
        use_managed_identity: Whether to use managed identity (True for Azure Automation)
...
def main():
    """
    Main entry point for the runbook.
    """
    try:
        write_to_output("Economic Data Pipeline starting...")
        ...
        # Run the pipeline with managed identity
        result = run_pipeline(
        ...
        # Output summary
        write_to_output("Economic Data Pipeline completed successfully.")
        write_to_output(f"Updated: {result['updated']['count']} datasets")
        write_to_output(f"No update needed: {result['no_update_needed']['count']} datasets")
        write_to_output(f"Failed: {result['failed']['count']} datasets")
        
        if result['failed']['count'] > 0:
            write_to_output(f"Failed datasets: {', '.join(result['failed']['datasets'])}")
        ...
        error_message = f"Error in Economic Data Pipeline: {str(e)}"
...
# This is required for Azure Automation
