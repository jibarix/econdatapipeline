...
def run_edb_scrapers(azure_connector: AzureConnector) -> Tuple[List[str], List[str], List[str]]:
    ...
    write_to_output("Starting EDB data scraper")
...
def run_fred_scrapers(azure_connector: AzureConnector) -> Tuple[List[str], List[str], List[str]]:
    ...
    write_to_output("Starting FRED data scraper")
...
def run_nyu_stern_scraper(azure_connector: AzureConnector) -> Tuple[List[str], List[str], List[str]]:
    ...
    write_to_output("Starting NYU Stern ERP data scraper")
...
def save_run_summary(azure_connector: AzureConnector, summary: Dict[str, Any]) -> None:
    ...
    try:
        ...
        write_to_output(f"Saved run summary to {container_name}/{blob_name}")
    except Exception as e:
        logger.error(f"Error saving run summary: {e}")

def run_data_collection(use_managed_identity: bool = True, storage_account: Optional[str] = None, 
    ...
    write_to_output("Initializing Azure Storage tables and containers")
    azure.initialize_tables()
    azure.initialize_containers()
    
    # Initialize revision tracking
    initialize_revision_tracking(azure)
    ...
    # Log summary
    write_to_output("\n\n" + "="*50)
    write_to_output(f"Scraping complete in {duration:.2f} seconds.")
    write_to_output(f"EDB: Updated: {len(edb_updated)}, No update needed: {len(edb_no_update)}, Failed: {len(edb_failed)}")
    write_to_output(f"FRED: Updated: {len(fred_updated)}, No update needed: {len(fred_no_update)}, Failed: {len(fred_failed)}")
    write_to_output(f"NYU: Updated: {len(nyu_updated)}, No update needed: {len(nyu_no_update)}, Failed: {len(nyu_failed)}")
    write_to_output(f"TOTAL: Updated: {len(all_updated)}, No update needed: {len(all_no_update)}, Failed: {len(all_failed)}")
    
    if all_updated:
        write_to_output(f"Updated scrapers: {', '.join(all_updated)}")
    if all_no_update:
        write_to_output(f"No update needed: {', '.join(all_no_update)}")
    if all_failed:
        write_to_output(f"Failed scrapers: {', '.join(all_failed)}")
...
# Main entry point for Azure Automation
def main():
    """
    Main entry point for Azure Automation.
    This function is called when the runbook is executed.
    """
    try:
        # Log start time
        start_time = datetime.utcnow()
        ...
        # Get environment variables set in Automation account
        ...
        # Run the data collection with managed identity
        ...
        return {
            ...
            "no_update_needed": result["no_update_needed"]["count"],
